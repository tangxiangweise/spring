...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		/**
		 * 如果是基础设施类(Pointcut,Advice,Advisor 等接口的实现类)，或是应该跳过的类，则不应该生成代理，此时直接返回bean
		 */
		...
		/**
		 * 若specificInterceptors != null ,即specificInterceptors != DO_NOT_PROXY，则为bean生成代理对象，否则直接返回bean
		 */
		if (specificInterceptors != DO_NOT_PROXY) {
			...
			this.proxyTypes.put(cacheKey, proxy.getClass());
			/**
			 * 返回代理对象，此时IOC容器输入bean，得到proxy。此时，beanName对应的bean是代理对象。而非原始的bean
			 */
			return proxy;
		}
	...
	protected Object createProxy(
		...
		/**
		 * 默认配置下或用户显式配置 proxy-target-class = "false" 时，
		...
		if (!proxyFactory.isProxyTargetClass()) {
			if (shouldProxyTargetClass(beanClass, beanName)) {
				...
				/**
				 * 检测beanClass 是否实现了接口，若未实现，则将proxyFactory的成员变量 proxyTargetClass 设为 true
				 */
